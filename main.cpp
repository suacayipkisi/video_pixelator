...
constexpr int LOW_WIDTH = 32;
constexpr int LOW_HEIGHT = 18;
constexpr int HIGH_WIDTH = 1920;
constexpr int HIGH_HEIGHT = 1080;

int main(int argc, char *argv[]) {
    if (argc < 3) {
        std::cerr << "Method of use: " << argv[0] << " <input_video> <output_video>\n";
        return -1;
    }
    ...
    if (avformat_open_input(&in_format_ctx, input_filename, nullptr, nullptr) < 0) {
        std::cerr << "Input video could not be opened.\n";
        return -1;
    }

    if (avformat_find_stream_info(in_format_ctx, nullptr) < 0) return -1;
    ...
    if (video_stream_idx == -1) {
        std::cerr << "Video stream could not be finded.\n";
        return -1;
    }
    ...
    out_codec_ctx->framerate = av_guess_frame_rate(in_format_ctx, in_format_ctx->streams[video_stream_idx], nullptr);
    ...
    if (audio_stream_idx != -1) {
        out_audio_stream = avformat_new_stream(out_format_ctx, nullptr);
    ...
    if (!(out_format_ctx->oformat->flags & AVFMT_NOFILE)) {
        if (avio_open(&out_format_ctx->pb, output_filename, AVIO_FLAG_WRITE) < 0) {
            std::cerr << "Output file could not be opened in write mode.\n";
            return -1;
        }
    }

    if (avformat_write_header(out_format_ctx, nullptr) < 0) {
        std::cerr << "Header could not be written. There is a container or codec mismatch.\n";
        return -1;
    }

    // Scaler
    SwsContext *sws_to_low = sws_getContext(
        in_codec_ctx->width, in_codec_ctx->height, in_codec_ctx->pix_fmt,
        LOW_WIDTH, LOW_HEIGHT, AV_PIX_FMT_YUV420P, SWS_POINT, nullptr, nullptr, nullptr
    );
    SwsContext *sws_to_high = sws_getContext(
        LOW_WIDTH, LOW_HEIGHT, AV_PIX_FMT_YUV420P,
        HIGH_WIDTH, HIGH_HEIGHT, AV_PIX_FMT_YUV420P, SWS_POINT, nullptr, nullptr, nullptr
    );
    ...
    while (av_read_frame(in_format_ctx, packet) >= 0) {
        if (packet->stream_index == video_stream_idx) {
            if (avcodec_send_packet(in_codec_ctx, packet) >= 0) {
                while (avcodec_receive_frame(in_codec_ctx, in_frame) >= 0) {
                    sws_scale(sws_to_low, static_cast<const uint8_t *const *>(in_frame->data), in_frame->linesize, 0,
                              in_codec_ctx->height, low_frame->data, low_frame->linesize);

                    sws_scale(sws_to_high, static_cast<const uint8_t *const *>(low_frame->data), low_frame->linesize, 0,
                              LOW_HEIGHT, out_frame->data, out_frame->linesize);

                    // Recalculating timestamps consistently.
                    out_frame->pts = av_rescale_q(video_frame_counter++, AVRational{1, static_cast<int>(av_q2d(out_codec_ctx->framerate))}, out_codec_ctx->time_base);
        ...
        } else if (packet->stream_index == audio_stream_idx && out_audio_stream != nullptr) {
